import { Address, Cell, toNano, TonClient, fromNano, Wallet, TonClient4, beginCell, StackItem, StackInt, StackSlice, StackCell } from "ton";
...
const client4 = new TonClient4({ endpoint: "https://mainnet-v4.tonhubapi.com" });
let LATEST_BLOCK = 0;
const getLatestBlock = async (): Promise<number> => {
    return new Promise(async (resolve) => {
        LATEST_BLOCK = (await client4.getLastBlock()).last.seqno;
        resolve(LATEST_BLOCK);
    });
};
let waitingForLatestBlock: Promise<number> = getLatestBlock();

setInterval(async () => {
    console.log("interval");

    LATEST_BLOCK = await (await client4.getLastBlock()).last.seqno;
}, 10000);
...
const callGetMethodV4 = async (address: Address, method: string, params: any) => {
    if (!LATEST_BLOCK) {
        LATEST_BLOCK = await waitingForLatestBlock;
    }
    let argsV4: StackItem[] = [];
    params.forEach((element: any[]) => {
        if (element[0] == "num") {
            argsV4.push({ type: "int", value: new BN(element[1]) });
        }
        if (element[0] == "tvm.Slice") {
            argsV4.push({ type: "slice", cell: beginCell().storeAddress(Address.parse(element[1])).endCell() });
        }
    });

    return client4.runMethod(LATEST_BLOCK, address, method, argsV4);
};
...
export async function _getJettonBalance(jettonWallet: Address, minterAddress?: Address) {
    try {
        ...
        let res = await callGetMethodV4(jettonWallet, "get_wallet_data", []);
        const balance = (res.result[0] as StackInt).value;
        const walletOwner = (res.result[1] as StackSlice).cell.beginParse().readAddress();
        const jettonMaster = (res.result[2] as StackSlice).cell.beginParse().readAddress();
        return {
            balance,
            walletOwner,
            jettonMaster,
...
async function getAmountOut(minterAddress: Address, amountIn: BN, reserveIn: BN, reserveOut: BN) {
    ...
    let res = await callGetMethodV4(minterAddress, "get_amount_out", [
        ["num", amountIn.toString()],
        ["num", reserveIn.toString()],
        ["num", reserveOut.toString()],
    ]);

    return (res.result[0] as StackInt).value;
}
...
async function getAmountIn2(minterAddress: Address, amountIn: BN, reserveIn: BN, reserveOut: BN) {
    ...
    let res = await callGetMethodV4(minterAddress, "get_amount_in", [
        ["num", amountIn.toString()],
        ["num", reserveIn.toString()],
        ["num", reserveOut.toString()],
    ]);
...
export async function getPoolData(ammMinter: Address) {
    let res = await callGetMethodV4(ammMinter, "get_jetton_data", []);

    const totalSupply = (res.result[0] as StackInt).value;
    const mintable = res.result[1] as StackInt;
    const jettonWalletAddressBytes = (res.result[2] as StackSlice).cell.beginParse().readAddress();
    const tonReserves = (res.result[3] as StackInt).value;
    const tokenReserves = (res.result[4] as StackInt).value;

    return {
        totalSupply,
        jettonWalletAddress: jettonWalletAddressBytes,
...
export async function getTokenData(jettonAddress: Address) {
    let jettonDataRes = await callGetMethodV4(jettonAddress, "get_jetton_data", []);
    const totalSupply = (jettonDataRes.result[0] as StackInt).value;
    const owner = (jettonDataRes.result[2] as StackSlice).cell.beginParse().readAddress();
    let cell = (jettonDataRes.result[3] as StackCell).cell;
...
export const generateSellLink = async (token: string, tokenAmount: string, tonAmount: string) => {
    ...
    console.log(`toNano(tonAmount).mul(new BN(995)).div(new BN(1000)) ${toNano(tonAmount).mul(new BN(995)).div(new BN(1000))}`);
