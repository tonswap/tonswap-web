...
export class WalletService {
    ...
    async requestTransaction<S>(adapterId: string, session: S, request: TransactionRequest): Promise<void | boolean | string> {
        const adapter = this.adapters.get(adapterId) as WalletAdapter<S>;
...
walletService.registerAdapter(Adapters.TON_KEEPER, new TonKeeperWalletAdapter());
